...
const BlogCard = ({blog}) => {
    const {title, author, description, category, image, _id} = blog;
    ...
    <>
        <div onClick={()=> navigate(`/blog/${_id}`)} className='w-full rounded-lg overflow-hidden shadow hover:scale-102 hover:shadow-primary/25 duration-300 cursor-pointer'>
            ...
            <div className='p-5'>
                ...
                <p className="inline-block py-1 px-4 rounded-full mb-6 border text-sm border-primary/35 bg-primary/5 font-medium text-primary">{author}</p>
            </div>
        </div>
